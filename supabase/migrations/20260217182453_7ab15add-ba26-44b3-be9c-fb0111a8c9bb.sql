...
-- Create detasseling_records table
CREATE TABLE public.detasseling_records (
  ...
  gleba_id uuid NULL,
  shift text NULL,
  area_worked_ha numeric NOT NULL,
  method text NOT NULL,
  team_size integer NULL,
  yield_per_person_ha numeric NULL,
  machine_id text NULL,
  machine_hours numeric NULL,
  machine_yield_ha_h numeric NULL,
  machine_speed_kmh numeric NULL,
  pct_detasseled_this_pass numeric NOT NULL,
  pct_remaining_after numeric NOT NULL,
  tassel_height text NULL,
  difficulties text[] NULL,
  non_conformities text NULL,
  photos text[] NULL,
...
-- Enable RLS
ALTER TABLE public.detasseling_records ENABLE ROW LEVEL SECURITY;

-- RLS policies following existing pattern
CREATE POLICY "rbac_select" ON public.detasseling_records
  AS RESTRICTIVE FOR SELECT USING (
    deleted_at IS NULL AND (
      ...
      OR (has_role(auth.uid(), 'client'::app_role) AND EXISTS (
        ...
        WHERE pc.id = detasseling_records.cycle_id AND pc.client_id = user_client_id() AND pc.deleted_at IS NULL
      ))
    )
  );
...
CREATE POLICY "rbac_update" ON public.detasseling_records
...
BEGIN
  ...
  ELSIF _table_name = 'detasseling_records' THEN
    UPDATE public.detasseling_records SET deleted_at = now() WHERE id = _record_id AND deleted_at IS NULL;
  ELSE
...
CREATE TRIGGER update_detasseling_records_updated_at
  BEFORE UPDATE ON public.detasseling_records
  FOR EACH ROW
