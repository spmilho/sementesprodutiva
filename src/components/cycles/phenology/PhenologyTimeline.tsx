...
// VT is index 8 - everything before is vegetative
const VT_INDEX = ALL_STAGES.indexOf("VT");

type ParentalFilter = "female" | "male" | "both";
...
function buildStageInfos(records: any[], filter: ParentalFilter, plantingDate?: string | null): StageInfo[] {
  ...
  // Build map: stage → earliest record
  ...
  // Find latest registered stage index
  let lastIdx = -1;
...
export default function PhenologyTimeline({ records, onClickFuture, onClickPast, plantingDate }: PhenologyTimelineProps) {
  const [filter, setFilter] = useState<ParentalFilter>("female");

  const femaleInfos = useMemo(() => buildStageInfos(records, "female", plantingDate), [records, plantingDate]);
  const maleInfos = useMemo(() => buildStageInfos(records, "male", plantingDate), [records, plantingDate]);
  const showBoth = filter === "both";

  const renderTimeline = (infos: StageInfo[], label?: string, labelColor?: string) => {
    // Find index of last registered stage for progress line
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex rounded-lg border border-border overflow-hidden">
          {(["female", "male", "both"] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={cn(
            ...
            >
              {f === "female" ? "🟣 Fêmea" : f === "male" ? "🔵 Macho" : "Ambos"}
            </button>
          ))}
        </div>
      </div>
      ...
      {showBoth ? (
        <div className="space-y-4">
          {renderTimeline(femaleInfos, "Fêmea", "bg-pink-50 text-pink-700 border-pink-200 dark:bg-pink-900/30 dark:text-pink-400 dark:border-pink-800")}
          {renderTimeline(maleInfos, "Macho", "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800")}
        </div>
      ) : filter === "female" ? (
        renderTimeline(femaleInfos)
      ) : (
        renderTimeline(maleInfos)
      )}
    </div>
  );
}
