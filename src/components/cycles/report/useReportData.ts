...
export async function fetchReportData(cycleId: string, cycle: any): Promise<ReportData> {
  ...
  const [
    ...
    sb.from("roguing_evaluations").select("*").eq("cycle_id", cycleId).is("deleted_at", null).order("evaluation_date"),
    sb.from("roguing_requests").select("*").eq("cycle_id", cycleId).is("deleted_at", null).order("request_date"),
    sb.from("roguing_executions").select("*").eq("cycle_id", cycleId).is("deleted_at", null).order("execution_date"),
    sb.from("planting_cv_records").select("*").eq("cycle_id", cycleId).is("deleted_at", null).order("type"),
    sb.from("stand_cv_records").select("*").eq("cycle_id", cycleId).is("deleted_at", null).order("type"),
  ...
  return {
    ...
    roguingEvaluations: roguingEvalRes.data || [],
    roguingRequests: roguingReqRes.data || [],
    roguingExecutions: roguingExecRes.data || [],
    plantingCvRecords: plantingCvRes.data || [],
    standCvRecords: standCvRes.data || [],
