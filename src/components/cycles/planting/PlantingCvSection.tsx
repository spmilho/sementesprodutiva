...
import { getCvLabel, PLANTING_TYPES } from "./planting-utils";

interface Props {
  cycleId: string;
  orgId: string;
  femaleMaleRatio: string; // e.g. "4F:2M"
}
...
export default function PlantingCvSection({ cycleId, orgId, femaleMaleRatio }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) {
        const { error } = await (supabase as any)
          .from("planting_cv_records")
          .update({ cv_percent: val })
          .eq("id", editingId);
        if (error) throw error;
      } else {
        // Upsert: check if record exists for this type
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["planting_cv_records", cycleId] });
      toast.success("CV% registrado com sucesso");
      setDialogOpen(false);
      setEditingId(null);
      setCvValue("");
      setSelectedType("");
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-foreground flex items-center gap-2">
          <BarChart3 className="h-4 w-4" /> CV% de Semeadura
        </h4>
      ...
      {cvRecords.length > 0 ? (
              ...
              return (
                <Card key={t.value} className="border-dashed">
                  <CardContent className="p-4 flex flex-col items-center justify-center text-center min-h-[80px]">
                    ...
                    <Button size="sm" variant="ghost" className="text-xs" onClick={() => { setSelectedType(t.value); setCvValue(""); setEditingId(null); setDialogOpen(true); }}>
                      + Registrar
                    </Button>
                  </CardContent>
                </Card>
        ...
        <div className="text-center py-6 text-sm text-muted-foreground border border-dashed rounded-lg">
          Nenhum CV% registrado. Clique em "Registrar CV%" para adicionar.
        </div>
      )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{editingId ? "Editar" : "Registrar"} CV% de Semeadura</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            ...
            <Button className="w-full" onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending || !selectedType || !cvValue}>
